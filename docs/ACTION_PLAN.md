...
| **N-24** | **The admin scope selector defaults to `Vendor` rather than forcing a choice.** The surviving form preselects the first scope in a visible dropdown; the discarded parallel implementation required an explicit pick with an empty "Choose…" first option and refused to submit without one. Both satisfy N-20(b)'s letter, but **the entire reason N-20(b) exists is that the *server* silently defaults to `Vendor`** — a UI that reintroduces the same silent default one layer up is weaker than the intent, and a type created as `Vendor` by accident can never back a shipment upload and cannot be re-scoped afterwards (D-34). Small, but it is DR-6's failure mode returning by the back door. Fix is one empty first option plus a required-check. |
...
| **N-23** | **Running the API from source needs `Jwt__SigningKey` in the environment.** Carried from the parallel pass, where it cost real time. `appsettings.json` ships it as `""` and only Docker supplies it, so `dotnet run` starts, connects to the database, and then **500s on login** with `IDX10703: key length is zero` — an error that reads like a credentials problem and is not. Cheap fixes: user-secrets, or an `appsettings.Development.json`. Worth doing before it catches anyone else. |
...
| **N-17, N-18** | **N-18 unchanged from §15.5. N-17 now has a precise root cause**, contributed by the parallel pass. The collision is a **native `postgresql-x64-18` Windows service and Docker's port proxy both bound to `0.0.0.0:5432`**, with the native service winning host connections — so `dotnet run` fails `28P01 password authentication failed` against credentials that are entirely correct, which reads as a secrets problem and is not. Verified by testing the same credentials on both ports: 5432 rejects, 55432 succeeds. The 55432 workaround was used again this pass via a scratchpad-only compose override — **still not committed**, so the repo's compose files assume a free 5432 and the published port vanishes if the stack is recreated. Settle before the deployment track: E2-07's `dotnet ef database update` path remains unproven on this machine. **The local `appsettings.json` edit carrying `Port=55432` and a dev password is parked in `git stash@{0}` ("local appsettings (machine-specific)")** — it belongs to neither branch and is the same unreproducible workaround this item tracks; resolve the two together or drop the stash. |
